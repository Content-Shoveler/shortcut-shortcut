...
# Add architecture information to the Info.plist file for Intel compatibility
echo "📝 Setting architecture to x86_64 in Info.plist..."
PLIST_BUDDY="/usr/libexec/PlistBuddy"
if [ -x "$PLIST_BUDDY" ]; then
  "$PLIST_BUDDY" -c "Add :LSArchitecturePriority array" "${APP_DIR}/Contents/Info.plist" 2>/dev/null || true
  "$PLIST_BUDDY" -c "Add :LSArchitecturePriority:0 string x86_64" "${APP_DIR}/Contents/Info.plist" 2>/dev/null || true
else
  echo "⚠️ PlistBuddy not found, manually add x86_64 to LSArchitecturePriority in Info.plist if needed"
fi
...
cd dist/release
zip -r ../Shortcut-Epic-Templates-Intel-macOS.zip "${APP_NAME}.app"
cd ../..
...
echo "   - Zip: dist/Shortcut-Epic-Templates-Intel-macOS.zip"
echo ""
